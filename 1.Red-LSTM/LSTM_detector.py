...
def plot_save_acc_loss(filename,history):
    ...
    plt.savefig('plots/' + filename + '_acc.png')
    plt.close()
    ...
    plt.xlabel('Época')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig('plots/' + filename +  '_loss.png')
    plt.close()
...
def create_model():
    ...
    model = Sequential()
    model.add(LSTM(100, return_sequences=True, input_shape=(250,136)))
...
def create_bi_model():
    ...
    print ('Creando el modelo.')
    model = Sequential()
    model.add(Bidirectional(LSTM(100), input_shape=(250,136)))
    model.add(Dropout(0.2))
    model.add(Dense(1, activation='sigmoid'))
    print ('Compilando el modelo.')
    model.compile(loss='binary_crossentropy',
...
def main():
    ...
    history = train_model(model, data, labels, 100)
