...
def test_load_landmarks():
    data = ld.load_landmarks(Path('./media/dadepe/Elements/Copia-Desktop/DeepFakeDetector.bak/landmarks/fake'), 3)
    if data is not None:
...
def test_bi_model_compile():
    model = ld.create_bi_model()
    if model is not None:
        print('El modelo compila correctamente.')
        return model
...
def test_data_treatment():
    data,labels = ld.data_treatment(Path('/media/dadepe/Elements/Copia-Desktop/DeepFakeDetector.bak/landmarks/'), 3)
...
def test_train():
    model = ld.create_model()
    data,labels = ld.data_treatment(Path('./landmarks'), 1)
    history = ld.train_model(model, data, labels,100)
...
def generate_outcomes():
    predictions,labels = ld.load_predictions(Path('Results/preds-out-dict.json'))
